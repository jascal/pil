...
def load_records(
    dump: Path, readout: Readout, emb: np.ndarray, n_layer: int, cache: Path, predict=None
) -> list[ExitRecord]:
    if cache.exists():
    ...
    recs = list(process_records(iter_dump(dump), readout, emb, n_layer, predict=predict))
