...
Qwen2.5 (0.5B--3B), across five languages and an easy$\to$hard difficulty dial, we
find that routing is strongly \emph{rank-concentrated}---the decode rides a stable,
identifiable, late-layer, MLP-dominated circuit of $\sim$7--14 effective blocks out
of 49--73, with the slack \emph{growing} with scale and difficulty. The same routing
features carry substantial \emph{coherence slack}, sitting 2--3$\times$ above the
information-theoretic (Welch) floor where a task-specialized learner packs to
$\sim$1$\times$. These are structural properties of routing invisible to accuracy or
loss curves, and they behave lawfully across scale, language, and difficulty.
%
...
backbone with zero \texttt{sorry}: exact softmax/decode recovery, a decode-separation
lemma with a $\gamma$-code packing bound, and a generator-rank bound.
%
...
packs routing features near the Welch floor. We release the \texttt{fieldrun}
...
that the formal and empirical content never blur: \proved{} (machine-checked, zero
\texttt{sorry}, in the companion \texttt{i-orca} corpus~\cite{iorca}), \measured{}
(empirical, this work or \texttt{fieldrun}), and \open{} (conjecture). Our
contributions are:
...
\item \textbf{A real-model validation} (\S\ref{sec:real}) over Qwen2.5
(0.5B--3B)~\cite{qwen2025}, five languages, and an easy$\to$hard difficulty dial,
showing rank-concentrated routing on a stable late-layer MLP circuit and 2--3$\times$
coherence slack, with the structural half of the theory transferring cleanly and the
near-optimal-packing half remaining synthetic-specific.
...
\begin{table}[t]
\caption{The machine-checked backbone of PIC and TRC (all \proved, zero
\texttt{sorry} in the \texttt{i-orca} corpus~\cite{iorca}). Empirical claims are
tagged \measured{} in situ; conjectures \open.}
\label{tab:proved}
\small
...
decode $1.00$ of the time, on every model, corpus, and language
(\S\ref{sec:seam}).
\item \textbf{Rank concentration on a stable circuit.} The decode rides $\sim$7--14
effective blocks out of 49--73---a consistent, late-layer, MLP-dominated read-out
circuit---and the slack \emph{grows} with scale (\S\ref{sec:rankreal}).
\item \textbf{Coherence slack.} Routing features sit 2--3$\times$ above the Welch
floor, where a task-specialized learner packs to $\sim$1$\times$
(\S\ref{sec:cohreal}).
\item \textbf{Difficulty increases slack via fallback.} Harder inputs route through
\emph{fewer}, looser blocks, not more---the opposite of the naive expectation
(\S\ref{sec:diffreal}).
\end{enumerate}
...
Qwen2.5 base 0.5B/1.5B/3B and 0.5B-Instruct/Coder ($\nb=49/57/73$). A replication on
the Pythia scaling suite~\cite{biderman2023pythia} is in progress and slots into
Tables~\ref{tab:scale}--\ref{tab:lang} as additional rows.
...
(Fig.~\ref{fig:rank}, Table~\ref{tab:scale}). Strikingly, the effective count is
\emph{$\sim$constant ($\sim$12) regardless of scale}, so as $\nb$ grows the rank
\emph{slack} grows (PR$/\nb$ $0.22\to0.16$). This is the empirical face of
Theorem~\ref{thm:rank}: real models use only a fraction of the available routing
...
concentration; $\mu/$Welch $\gg1$ is coherence slack. \emph{(Pythia rows to follow.)}}
\label{tab:scale}
\small
...
\subsection{Real models route with coherence slack}
\label{sec:cohreal}
Treating each position's decode-vs-runner-up direction in block space as a routing
feature, their mutual coherence $\mu$ sits 2--3$\times$ above the Welch floor
$\sqrt{(N-\nb)/(\nb(N-1))}$ in every condition ($\mu\approx0.26$--$0.41$;
Fig.~\ref{fig:coh}). This is the one synthetic finding that does \emph{not} transfer:
a task-specialized learner packs its routing features near Welch-optimal coherence
($\sim$1$\times$), whereas a general pretrained model on this imposed
routing-feature basis is 2--3$\times$ looser. The structural bound
(Theorem~\ref{thm:welch}: $n>\nb$ forces coherence $\ge$ Welch) holds---the floor is
real---but pretrained models leave slack above it. This gap is the clearest
quantitative signal in the real-model data.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/coherence_slack.pdf}
\caption{Coherence relative to the Welch floor across every real condition. A
task-specialized synthetic learner packs to $\sim$1$\times$ (dashed line); real
Qwen2.5 models sit 2--3.6$\times$ above it across scale, difficulty, and language.}
\label{fig:coh}
\end{figure}
...
The behavior is qualitatively consistent across five languages
(Table~\ref{tab:lang}): all reconstruct $1.00$, with $\sim$7--11 effective blocks and
$\mu\approx0.27$--0.36. Non-English is slightly \emph{more} concentrated and looser,
consistent with the difficulty finding (marginally lower fluency $\to$ more
fallback); Chinese matches English coherence ($\mu{=}0.269$) despite the script.
(Non-English $N$ is small, 96--127, so $\mu$ is noisier; the qualitative picture is
the signal.)

\begin{table}[t]
\caption{Multilingual routing (Qwen2.5-0.5B-Instruct, matched content). Qualitatively
robust; non-English is slightly more concentrated and looser.}
\label{tab:lang}
\small
\begin{tabular}{lccc}
\toprule
...
(\S\ref{sec:scored}), and real models confirm large rank slack that grows with scale
...
Real pretrained models, by contrast, leave coherence slack (2--3$\times$ the floor)
on the imposed routing-feature basis, while still being strongly rank-concentrated
on a stable late-MLP circuit. The asymmetry between what transfers (rank
concentration) and what does not (near-optimal packing) is itself informative: a
general model is not optimized for any single routing decomposition, so it does not
pay to pack tightly, whereas a task-specialized learner does. Read this way, the
synthetic nulls and the real-model slack are two views of one fact---tight routing is
a property of \emph{specialization}---and PIC is the instrument that makes the fact
measurable.
...
A network can represent more features than it has dimensions only by accepting
interference, and that interference is bounded below by coherence measures such as
the Welch bound~\cite{welch1974lower}. This is the lens of toy-model and
complexity-theoretic work on
superposition~\cite{elhage2022superposition,adler2024superposition}, which has
focused on \emph{hidden representations}. We extend the same accounting to the
...
property that \emph{grows} with scale, rather than imposing low-rank structure for
efficiency.
...
of that additive decomposition we add a geometric and logical semantics---incidence
calculus plus tropical decode---and a capacity theory. We also modernize classical
incidence calculus~\cite{bundy1985incidence,liu1996incidence} for the inner-product
structure of transformer residuals, replacing world-sets with signed incidences and
...
Theorem~\ref{thm:sep}. Finally, results to date span the Qwen2.5 family; the Pythia
replication (\S\ref{sec:real}) will test family-independence directly.
...
hugely slack, and SGD already packs near the floor. Real-model analysis confirms
strongly rank-concentrated routing on a stable late-MLP circuit with substantial,
difficulty-amplified coherence slack.
...
pressure; and the in-progress Pythia replication to establish family-independence of
the rank-concentration and coherence-slack laws. All code, formal certificates, and
results are available in the \texttt{fieldrun}, \texttt{i-orca}, and \texttt{pil}
repositories.
