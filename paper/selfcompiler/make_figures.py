...
# ---- Fig 4: campaign 2 -- the crystallization threshold dissolves (PRs #15-#22) ----
c2 = ["code", "isab.", "py", "legal", "de", "math", "wt103", "wiki"]
before = [106.1, 102.1, 106.6, 102.4, 93.1, 91.2, 96.2, 94.5]   # domain study (20e8bd6)
after = [104.7, 101.7, 108.1, 102.0, 97.3, 96.4, 98.0, 99.5]    # full stack (cf26ce4)
x = np.arange(len(c2))
fig, ax = plt.subplots(figsize=(6.6, 2.4))
ax.bar(x - 0.19, before, 0.38, label="domain study (memorization library)", color="#cbd5e0")
ax.bar(x + 0.19, after, 0.38, label="full stack (derived roles + learned sets)", color="#276749")
ax.axhline(100, color="#c05621", lw=0.8, ls="--")
ax.set_xticks(x, c2, fontsize=8)
ax.set_ylabel("crystallization (%)")
ax.set_ylim(85, 112)
ax.legend(frameon=False, fontsize=7.5)
fig.tight_layout()
fig.savefig(OUT / "campaign2.pdf")
plt.close(fig)
print("fig 4 ->", OUT / "campaign2.pdf")
